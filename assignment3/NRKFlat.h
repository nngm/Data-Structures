...
NRKFlat::NRKFlat(enum overflow_handle _flag, float _alpha, unsigned int _filter_size) : FlatHash(_flag, _alpha)
{
  ...
  // Write your code
  counters = new unsigned int[filter_size]{0, };
}
...
bool NRKFlat::filter(const unsigned int key)
{
  ...
  // Write your code
  return counters[h1 % filter_size] && counters[h2 % filter_size] && counters[h3 % filter_size];
}

int NRKFlat::insert(const unsigned int key)
{
  // Write your code
  unsigned int ts = this->getTableSize();
  int time_cost = FlatHash::insert(key);
  if (time_cost > 0) {
    unsigned int h1, h2, h3;
    getMMHashValue(key, h1, h2, h3);

    counters[h1 % filter_size]++; counters[h2 % filter_size]++; counters[h3 % filter_size]++;

    if (ts != this->getTableSize()) {
      delete[] counters;
      counters = new unsigned int[filter_size * 2]{0, };
      filter_size += filter_size;

      unsigned int *hashtable = this->getTable();
      for (size_t i = 0; i < this->getTableSize(); i++) {
        if (hashtable[i] != 0 && hashtable[i] != 1234567) {
...
{
  // Write your code
  return filter(key) ? FlatHash::search(key) : 0;
}
